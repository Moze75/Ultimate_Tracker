...
/* ==== NOUVEAU: RÈGLES D’AFFICHAGE DES NIVEAUX AUTORISÉS D’APRÈS D&D 5e ==== */
...
const getHighestAllowedSlotLevel = (casterType: CasterType, level: number): number => {
  ...
  if (casterType === 'half') {
    // Caster level effectif = floor(level / 2), ensuite même logique de plus haut niveau
    const eff = Math.floor(level / 2);
    // Demi-lanceurs ne dépassent jamais 5e niveau de slot
...
function SpellCard({ 
  ...
  return (
    ...
    >
      ...
      {/* Popup de confirmation de suppression - Centré sur toute la carte */}
      {showDeleteConfirm && (
          ...
          <div className="bg-gray-900 border border-red-500/50 rounded-lg p-4 shadow-xl min-w-[250px] mx-4">
            ...
            <div className="flex gap-3 justify-center">
              <button
                ...
                className="px-3 py-2 text-sm bg-gray-700 hover:bg-gray-600 text-gray-300 rounded transition-colors"
              >
                Annuler
              </button>
              <button
      ...
      {/* Détails étendus - Section complètement adaptative */}
      {isExpanded && (
        <div className="border-t border-gray-700/50 bg-gray-900/50">
          <div className="p-3 space-y-4 animate-in slide-in-from-top duration-300">
            ...
            {/* Description - Hauteur automatique complète */}
...
export function KnownSpellsSection({ player, onUpdate }: KnownSpellsSectionProps) {
  ...
  // ==== NOUVEAU: Déterminer les niveaux autorisés par classe/niveau ====
  ...
  // Déterminer quels niveaux afficher:
  // - Cantrips si présents
  // - Niveaux 1..9 autorisés PAR les règles + avec slots>0 ou sorts présents
  const levelsToRender = useMemo(() => {
    const levels: string[] = [];
    if (groupedSpells['Tours de magie'] && groupedSpells['Tours de magie'].length > 0) {
      levels.push('Tours de magie');
    }
    for (let lvl = 1; lvl <= 9; lvl++) {
      if (!allowedLevelsSet.has(lvl)) continue; // borne D&D 5e
      ...
      <div className="p-3">
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {/* Rendre d'abord les cantrips s'ils existent, puis les niveaux 1..9 disponibles */}
